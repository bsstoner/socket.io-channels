var http = require('http')
  ...
  , channels = require('./../')
  , decode = require('./../support/Socket.IO-node/lib/socket.io/utils').decode
...
module.exports = {
  ...
  'test broadcasting to channel': function(){
    var _server = server()
      , _socket = socket(_server)
      , _channel = channels.listen(_socket, {});
      ...
      listen(_server, function(){
        ...
        _channel.on('connectedToChannel', function(cl, sessionInfo){
          _channel.broadcastToChannel('announcement',sessionInfo.channelId,{message:"Hi"})
        })
        ...
        _client1.onmessage = function(ev){
          if (!_client1._first){
            ...
            var d = JSON.parse(decode(ev.data)[0])
            assert.ok(d.message == "Hi");
            --trips || close();
          }
        }
        ...
        var connMsg = JSON.stringify({
          event: "connectToChannel",
          channelId: "testChannel1",
          session: {username: "user123" }
        })

        _socket.on('connection', function(cl){
          _channel.receive(cl,connMsg)
        })

      })
  },

  'test broadcasting is channel specific': function(){
    var _server = server()
      , _socket = socket(_server)
      , _channel = channels.listen(_socket, {});
      ...
      listen(_server, function(){
        ...
        _channel.on('connectedToChannel', function(cl, sessionInfo){
          _channel.broadcastToChannel('announcement',sessionInfo.channelId,{message:"Hi" + cl.sessionId})
        })
        ...
        _client1.onmessage = function(ev){
          if (!_client1._first){
            ...
            assert.ok(d.message == "Hi" + sessionId);
            --trips || close();
          }
        }
        ...
        _socket.on('connection', function(cl){
          connMsg.channelId = "testChannel" + cl.sessionId
          sessionIds.push(cl.sessionId)
          _channel.receive(cl,JSON.stringify(connMsg))
        })
